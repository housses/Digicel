...
trigger triggerSummarizeOpportunitiesUpdate on Opportunity (after update) {
    ...
    System.debug('Trigger is running: triggerSummarizeOpportunitiesUpdate'); 
        ...
        for(Date d: mapChargeTypeSummaryByDate.keySet()){
            ...
            for(Summary_by_Charge_Type__c chargeTypeSummary: listChargeTypeSummary){
                if(chargeTypeSummary.Summary__c == null)
                    chargeTypeSummary.Summary__c = (mapSummaryByCountry.containsKey(billingCountry)&&mapSummaryByCountry.get(billingCountry).containsKey(d)?mapSummaryByCountry.get(billingCountry).get(d).Id:null);
                if(mapSummaryByCountry.containsKey(billingCountry) && mapSummaryByCountry.get(billingCountry).containsKey(d) && mapUpdatedSummary.containsKey(mapSummaryByCountry.get(billingCountry).get(d).Id)){
